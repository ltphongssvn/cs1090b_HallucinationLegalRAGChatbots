...
#                       NO_JUPYTER=1 — skip entirely
write_repro_env() {
    _require_project_root
    if _is_dry_run; then
        _msg_dry_run "write reproducibility .env" "${PROJECT_ROOT}/.env"
        _msg_info "Would write: PYTHONHASHSEED=${REPRO_PYTHONHASHSEED} CUBLAS=${REPRO_CUBLAS_CFG} RANDOM_SEED=${RANDOM_SEED} TARGET_GPU_COUNT=${TARGET_GPU_COUNT}"
        step_end "write_repro_env" "DRY"; return
    fi
    echo " Writing reproducibility .env..."
    cat > "${PROJECT_ROOT}/.env" << ENVEOF
# .env — generated by setup.sh — do not edit manually
...
# GPU allocation — resolved at setup time from SLURM_GPUS_ON_NODE / nvidia-smi.
# Consumed by src/environment.run_preflight_checks (Check 1: exact GPU count match).
export TARGET_GPU_COUNT=${TARGET_GPU_COUNT}
export TARGET_VRAM_GB_MIN=${TARGET_VRAM_GB_MIN}
ENVEOF
    [ ! -f "${PROJECT_ROOT}/.env" ] && {
...
"
    if [ -n "${USER_CUDA_VISIBLE_DEVICES:-}" ]; then
        _msg_ok "TARGET_GPU_COUNT=${TARGET_GPU_COUNT} (resolved from CUDA_VISIBLE_DEVICES=${USER_CUDA_VISIBLE_DEVICES})"
    else
        _msg_ok "TARGET_GPU_COUNT=${TARGET_GPU_COUNT} (resolved from nvidia-smi)"
    fi
}
write_repro_module() {
    _require_repro_env
    if _is_dry_run; then
        ...
        _msg_info "Would inject: RANDOM_SEED=${RANDOM_SEED} CUBLAS=${REPRO_CUBLAS_CFG}"
        step_end "write_repro_module" "DRY"; return
    fi
    _require_python
    echo " Writing src/repro.py..."
...
if failed:
    ...
    for f in failed: print(f'    • {f}')
    print('\033[0;36m  Fix: Check ~/.bashrc, then: bash setup.sh\033[0m')
    sys.exit(1)
...
register_kernel() {
    # NO_JUPYTER and DRY_RUN guards BEFORE _require_python —
    # so dry-run / no-jupyter mode never requires a venv Python version check.
    ...
    if _is_dry_run; then
        _msg_dry_run "install Jupyter kernelspec" \
            "~/.local/share/jupyter/kernels/hallucination-legal-rag/ (HallucinationLegalRAG (${TARGET_PYTHON_VERSION}))"
        step_end "register_kernel" "DRY"; return
    fi
    # Only require venv Python for the actual install path
    _require_python
...
if 'hallucination-legal-rag' not in kernels:
    print('\033[0;31m  ✗ Kernel not found after registration\033[0m')
