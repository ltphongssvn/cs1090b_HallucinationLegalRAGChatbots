# scripts/patch_run_probe_docstring.py
# Project: HallucinationLegalRAGChatbots
# Path: cs1090b_HallucinationLegalRAGChatbots/scripts/patch_run_probe_docstring.py
"""One-shot in-place patcher for ``src.dataset_probe.run_probe``'s docstring.

Rewrites the existing ``run_probe()`` docstring to accurately document
``full_scan`` semantics: the Polars scan is always performed, and the
flag only controls whether loaded records are reservoir-subsampled to
``subset`` before gates run. The previous docstring incorrectly
implied ``full_scan`` controlled scan scope.

Idempotent via exact-match assertion: if the ``OLD`` block is not
found (already patched, or file changed), the script aborts with
:class:`AssertionError` rather than writing garbage.

Usage:
    python scripts/patch_run_probe_docstring.py
"""
...
assert OLD in content, "Target not found. Check exact bytes."
