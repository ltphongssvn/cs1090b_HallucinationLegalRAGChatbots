...
def _link_upstream_lineage() -> None:
    """Connect upstream RRF artifact to this run for W&B lineage.

    Calls ``run.use_artifact("baseline-rrf:latest")`` via
    :func:`src.wandb_lineage.link_input_artifacts` so the W&B lineage graph
    shows ``baseline-rrf -> reranker`` edge. No-op when wandb is offline /
    inactive.
    """
    from src.wandb_lineage import link_input_artifacts
    link_input_artifacts(["baseline-rrf:latest"], artifact_type="dataset")


def _log_output_artifact(out_dir: Path) -> None:
    """Log reranker output as W&B artifact for downstream RAG / eval lineage.

    Calls ``run.log_artifact(...)`` so RAG / judge runs can mark this
    reranker output as input via ``use_artifact``. No-op when wandb is
    offline / inactive.
    """
    try:
        import wandb
    ...
    if run is None or getattr(run, "offline", False):
        return
    art = wandb.Artifact("baseline-reranker", type="dataset")
