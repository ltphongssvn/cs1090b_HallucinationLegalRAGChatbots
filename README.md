...
## Appendix: DVC Workflow for Large Files and Datasets

The project tracks all files >100 MB through [DVC](https://dvc.org/) backed by an S3 remote (`s3-dvc`, bucket `cs1090b-hallucinationlegalragchatbots/dvc`, region `us-east-2`). This keeps the git repository small (<100 MB of code + small metadata) while ensuring every collaborator can reproduce the exact same datasets, embeddings, and retrieval outputs from any fresh clone.

### How the workflow operates

1. **Large artifacts are tracked with a `.dvc` pointer file in git.** Each pointer is a small (~200 byte) text file holding the SHA-256 content hash and the S3 path. The actual binary lives in `.dvc/cache/` locally and in `s3://cs1090b-hallucinationlegalragchatbots/dvc/` remotely.
2. **`.gitignore` rules (`data/**`) prevent git from ever tracking the binary data itself.** Only the pointer files are versioned.
3. **`dvc push` uploads local cache blobs to S3.** `dvc pull` downloads them on a fresh clone. Both commands use the pointer's content hash to avoid redundant transfer.
4. **The `scripts/verify_dvc.sh` utility audits reproducibility status.** It emits JSON (machine-readable) or a human report covering: remote configured, pointer count, cloud drift state, and any large files (>100 MB) that have escaped DVC tracking.

### Current DVC-tracked artifacts

| Path (pointer) | Size | Purpose |
|---|---|---|
| `data/raw/cl_bulk.dvc` | 58 GB | CourtListener bulk dumps (opinions 51 GB, dockets 4.6 GB, opinion-clusters 2.4 GB) - upstream source for corpus construction |
| `data/raw/cl_federal_appellate_bulk.dvc` | ~30 GB across 159 shards | Pre-filtered federal appellate opinions used as corpus-build input |
| `data/processed/baseline/corpus_chunks.jsonl.dvc` | 26 GB | Full chunked corpus: 7,813,273 chunks at 1024 tokens with 128-token overlap |
| `data/processed/baseline/corpus_chunks_opinion_sample.jsonl.dvc` | 4.1 GB | One-chunk-per-opinion subsample used by the MS3 BGE-M3 baseline (1,465,484 rows) |
| `data/processed/baseline/bm25_results.jsonl.dvc` | 229 MB | BM25 retrieval output: 45,000 queries × top-100 hits (DVC-tracked, results_hash captured in `bm25_summary.json`) |
| `data/processed/baseline/bge_m3_results.jsonl.dvc` | 233 MB | BGE-M3 merged retrieval output: 45,000 queries × top-100 hits |
| `data/processed/baseline/bge_m3_summary.json.dvc` | 674 B | BGE-M3 run summary (Pydantic-validated via `BaselineBgeM3Summary`) |
| `lepard_train_4000000_rev0194f95.jsonl.dvc` | 5.8 GB | LePaRD training data (4M source-quote-dest triples) |

Total DVC-managed data on S3: **~120 GB across 8 pointer groups.**

### Artifacts NOT tracked by DVC (by design)

Some large files are intentionally excluded from DVC tracking because they are **deterministically reproducible from already-tracked inputs**:

- `data/processed/baseline/bge_m3_index.rank{000,001,002,003}.faiss` (4 × 1.4 GB = 5.6 GB) - FAISS per-rank indices. Regenerated by `scripts/baseline_bge_m3.py` from the subsample corpus in ~14 hours on 4× L4 GPUs. Tracking them would duplicate storage cost for zero reproducibility benefit, since any clone can rebuild them from tracked inputs.
- `data/processed/baseline/bge_m3_index_meta.rank*.jsonl` (~15 MB × 4) - Parallel per-rank chunk metadata, derived from the FAISS indices.
- `data/processed/baseline/bge_m3_results.rank{000,001,002,003}.jsonl` (~244 MB × 4) - Pre-merge per-rank retrieval shards. The merged `bge_m3_results.jsonl` is what's tracked; per-rank shards are deterministic byproducts.

### Verifying reproducibility end-to-end

From a fresh clone on any machine with DVC and AWS credentials configured:

```bash
git clone git@github.com:ltphongssvn/cs1090b_HallucinationLegalRAGChatbots.git
cd cs1090b_HallucinationLegalRAGChatbots
uv sync                                    # install dependencies locked in uv.lock
dvc pull                                   # download all tracked binaries from S3
./scripts/verify_dvc.sh --strict --json    # audit DVC state, exit 1 if drift
```

The `verify_dvc.sh` script takes ~90 seconds and returns a JSON document of the form:
...
`cloud_drift: false` confirms that every `.dvc` pointer's content hash is present in S3 - meaning every collaborator can reconstruct the same datasets byte-for-byte.

### When to DVC-track a new artifact

The decision rule, enforced by `scripts/verify_dvc.sh` and a 19-test bats suite (`tests/shell/test_verify_dvc.bats`):

1. **File size ≥ 100 MB** AND
2. **Not deterministically reproducible from tracked inputs within ≤ 20 hours of compute** AND
3. **Intended to be shared across collaborators** (vs. a local-only experiment byproduct)

To add a new artifact:

```bash
dvc add path/to/large_file.jsonl      # creates path/to/large_file.jsonl.dvc
git add path/to/large_file.jsonl.dvc  # commit the pointer only
git commit -m "data: track large_file.jsonl (X GB) for reproducibility"
git push
dvc push                              # upload the binary to S3
```

### Monitoring DVC pipeline health

`scripts/verify_dvc.sh` supports three modes:

- **Default (permissive):** Exit 0, print human-readable report. Use this during development.
- **`--strict`:** Exit 1 on cloud drift. Use in CI pipelines or before MS submissions.
- **`--json`:** Machine-readable output for integration with experiment dashboards.

Additional flags:

- `VERIFY_DVC_SIZE_MB=50 ./scripts/verify_dvc.sh` - Override the 100 MB untracked-file threshold.
- `VERIFY_DVC_BIN=/custom/path/to/dvc ./scripts/verify_dvc.sh` - Use a custom DVC binary (useful in non-standard CI environments).

### What this guarantees

- **Reproducibility:** Any collaborator can reconstruct every dataset, embedding, and retrieval output from a fresh clone + `dvc pull`. All content hashes are verified against S3 on download.
- **Cost control:** Large binaries don't bloat the git repository. GitHub stays under the 1 GB soft limit; S3 carries the heavy data.
- **Drift detection:** `dvc status --cloud` (and the `verify_dvc.sh` wrapper) surface any local artifacts that have diverged from S3 - caught automatically before they become silent reproducibility bugs.
