...
#   Helpers prefixed with _ are private (not run_step targets).
#   Public step functions are thin orchestrators over private helpers.
#
...
# --- preflight helpers ---
_check_disk_space() {
    # SRP: only checks disk — nothing else
    local free_gb
    free_gb=$(df -BG "$PROJECT_ROOT" | awk 'NR==2 {gsub("G",""); print $4}')
    if [ "${free_gb:-0}" -lt "$TARGET_MIN_DISK_GB" ]; then
        echo "Disk: only ${free_gb}GB free on $PROJECT_ROOT, need ${TARGET_MIN_DISK_GB}GB"
        return 1
    fi
...
_check_nvidia_smi_present() {
    # SRP: only checks nvidia-smi binary presence
    if ! command -v nvidia-smi &>/dev/null; then
        echo "nvidia-smi not found — not a GPU node. Request a GPU allocation."
        return 1
    fi
...
_check_gpu_count_smi() {
    # SRP: only checks GPU count via nvidia-smi (no torch context required)
    local count
    count=$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | wc -l | xargs)
    if [ "${count:-0}" -lt "$TARGET_GPU_COUNT" ]; then
        echo "GPU count (nvidia-smi): detected ${count}, need ${TARGET_GPU_COUNT}. Check CUDA_VISIBLE_DEVICES."
        return 1
    fi
...
_check_gpu_name_smi() {
    # SRP: only checks GPU name substring via nvidia-smi
    local names
    names=$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | sort -u | tr '\n' ' ')
    if ! echo "$names" | grep -q "$TARGET_GPU_NAME"; then
        echo "GPU name (nvidia-smi): detected '${names}', expected '${TARGET_GPU_NAME}'. Wrong node."
        return 1
    fi
...
_check_driver_cuda_smi() {
    # SRP: only checks driver CUDA version — warns, does not fail (forward-compat)
    local driver_cuda
...
_check_lockfile_present() {
    # SRP: only checks pyproject.toml and uv.lock presence
    local ok=true
    [ ! -f "$PROJECT_ROOT/pyproject.toml" ] && { echo "pyproject.toml not found at $PROJECT_ROOT"; ok=false; }
    [ ! -f "$PROJECT_ROOT/uv.lock" ] && { echo "uv.lock not found — run: uv lock && git add uv.lock && git commit"; ok=false; }
    [ "$ok" = "true" ] && echo -e "  ${C_GREEN}✓${C_RESET} pyproject.toml + uv.lock: present"
    [ "$ok" = "false" ] && return 1
}

_check_uv_present() {
    # SRP: only checks uv binary presence
...
# --- detect_hardware helpers ---
_query_torch_hardware() {
    # SRP: only queries torch for GPU properties — returns JSON, no comparison
    $PYTHON -c "
import torch, json
result = {
...
_parse_detected_hardware() {
    # SRP: only parses JSON from _query_torch_hardware into shell vars
    local hw_json="$1"
...
_print_hardware_table() {
    # SRP: only formats and prints the detected vs target comparison table
    local hw_json="$1"
...
_compare_hardware_to_targets() {
    # SRP: only compares DETECTED_* to TARGET_* and sets HARDWARE_MATCH
    # Emits warnings — does NOT hard-fail (run_gpu_smoke_tests() does that)
...
# --- run_gpu_smoke_tests helpers ---
_assert_cuda_available() {
    # SRP: only asserts CUDA is available and torch runtime matches pinned wheel
    $PYTHON -c "
import torch
...
_assert_gpu_count() {
    # SRP: only asserts correct number of GPUs visible to torch
    $PYTHON -c "
import torch
...
_assert_per_gpu_specs() {
    # SRP: only asserts name, compute cap, and VRAM for each GPU
    $PYTHON -c "
import torch
...
_assert_gpu_tensor_op() {
    # SRP: only runs a functional tensor round-trip on GPU 0
    $PYTHON -c "
import torch
...
# --- write_manifest helpers ---
_collect_manifest_data() {
    # SRP: only collects all manifest data — returns JSON string
...
_write_manifest_file() {
    # SRP: only writes collected data to disk — no collection logic
...
# ===========================================================================
# Public step functions — thin orchestrators over private helpers
# ===========================================================================

preflight_fast_checks() {
    # Orchestrates all cheap pre-venv gates — collects failures, gates once
    echo " Running preflight fast checks (pre-venv, seconds)..."
    local failures=()

    _check_disk_space          || failures+=("$(  _check_disk_space 2>&1 | tail -1)")
    _check_uv_present          || failures+=("$(_check_uv_present 2>&1 | tail -1)")
    _check_lockfile_present    || failures+=("$(_check_lockfile_present 2>&1 | tail -1)")
    _check_nvidia_smi_present  || failures+=("$(_check_nvidia_smi_present 2>&1 | tail -1)")

    if command -v nvidia-smi &>/dev/null; then
        _check_gpu_count_smi   || failures+=("$(_check_gpu_count_smi 2>&1 | tail -1)")
        _check_gpu_name_smi    || failures+=("$(_check_gpu_name_smi 2>&1 | tail -1)")
        _check_driver_cuda_smi
    fi
...
check_lockfile() {
    [ ! -f "$PROJECT_ROOT/pyproject.toml" ] && { echo -e "${C_RED}ERROR${C_RESET}: pyproject.toml not found."; exit 1; }
...
log_gpu() {
    # Pre-venv: driver-level info only. torch details in detect_hardware().
...
detect_hardware() {
    # Orchestrates: query → parse → print → compare
    # Each responsibility is a separate private helper.
...
ensure_venv() {
    ...
    if [ -d "$PROJECT_ROOT/.venv" ]; then
        echo -e "  ${C_YELLOW}WARNING${C_RESET}: .venv exists but is NOT Python ${TARGET_PYTHON_VERSION} — will remove."
        ...
        echo "         Aborting in 5 seconds — Ctrl+C to cancel..."
        sleep 5
...
assert ver.startswith('2.') and 'cu' in ver
...
run_gpu_smoke_tests() {
    # Orchestrates four single-responsibility GPU checks
...
write_manifest() {
    # Orchestrates: collect data → write file (two separate responsibilities)
...
assert 'hallucination-legal-rag' in kernels,'Kernel not found'
...
run_step preflight_fast_checks   # FIRST: cheap gates — fail before expensive ops
run_step check_uv
...
run_step run_gpu_smoke_tests      # hard gate: four single-responsibility sub-checks
run_step write_manifest           # collect → write (two helpers)
