...
matplotlib renders to PNG for inline notebook display. MS3_PIPELINE_SPEC
is the canonical current-state + roadmap diagram.
...
def _layout_nodes(nodes: Sequence[dict[str, Any]], edges: Sequence[tuple[str, str]]) -> dict[str, tuple[float, float]]:
    ...
    for lyr, ids in by_layer.items():
        for i, nid in enumerate(sorted(ids)):
            y = -(i - (len(ids) - 1) / 2.0)  # center vertically
...
# ---------- canonical MS3 pipeline spec ----------


MS3_PIPELINE_SPEC: dict[str, Any] = {
    "stages": [
        # MS3 executed stages
        {"id": "corpus", "label": "CourtListener\nCorpus Chunks\n(7.8M)", "kind": "data"},
        {"id": "gold", "label": "LePaRD Gold Pairs\n(45K test + 2K val)", "kind": "data"},
        ...
        {"id": "rag", "label": "RAG Generation\n+ Hallucination Check", "kind": "future"},
    ],
    "edges": [
        # MS3 flow
        ("corpus", "bm25"),
