...
class TestVerifyOnlyCapCheck:
    def test_verify_only_raises_on_cap_mismatch(self, tmp_path):
        import pytest
...
class TestVerifyOnlyRowsWrittenCheck:
    def test_verify_only_raises_on_rows_written_mismatch(self, tmp_path):
        ...
        write_jsonl(iter(rows), out, cap=5,
                    revision="0194f95c3091acceab3b887c9b09ef432cf84052",
                    dataset="rmahari/LePaRD", split="train")
        m = _json.loads(_manifest_path(out).read_text())
        m["rows_written"] = 999
        _manifest_path(out).write_text(_json.dumps(m))
        with pytest.raises(ValueError, match="manifest mismatch"):
            write_jsonl(iter([]), out, cap=5, verify_only=True,
                        revision="0194f95c3091acceab3b887c9b09ef432cf84052",
                        dataset="rmahari/LePaRD", split="train")
